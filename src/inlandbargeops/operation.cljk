(ns inlandbargeops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (InlandBargeAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the InlandWaterwayFreightGovernor (:govern) and the rollout
  ...
  human inland-logistics coordinator (or, for a safety concern, the tow
  captain/lockmaster). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected).

  This actor coordinates INLAND WATERWAY PORT/LOGISTICS SCHEDULING ONLY
  -- it never navigates a barge or towboat, never operates lock
  infrastructure, and never overrides a tow captain's or lockmaster's
  safety judgment. See `inlandbargeops.governor` for the HARD scope
  invariants."
...
(defn build
  ...
  `inlandbargeops.store/Store`).
  opts:
    :advisor      -- an `inlandbargeops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; InlandBargeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; InlandWaterwayFreightGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :vessel-id (:vessel-id request)
                        :reason (or reason
      ...
      ;; Approval handoff -- paused by interrupt-before; a human inland-
      ;; logistics coordinator (or tow captain/lockmaster for a safety
      ;; concern) resumes with :approval. Then route commit/hold.
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :vessel-id (:vessel-id request) :by (:by approval)}]}
            {:disposition :hold
